...
void macPs::actDeal(bool)
{
	...
	for (int j = 10; j < 21; j++)
	{
		...
		if (j <13)
		{
			relay_data[j] = (m_unPwrVal >> (8 * (j - 10))) & 0xff;
		}
	...
	if (rv != relay_data.length())
	{
		g_disText << "Hvs : Send Config Message Failed !";
		uSocket->disconnect();
		delete uSocket;
		activeButt->setEnabled(true);
		return;
	...
	if (rv != relay_data.length())
	{
		g_disText << "Hvs : Send Active Message Failed !";
	...
	{
		g_disText << weChinese2LocalCode("电源模拟器 配置激活已发送") + " : " + QString("0b%1").arg(m_unPwrVal,0,2);
	}
