...
#define UDPREADWAIT_TIMEOUT 200 //MS
...
void macSP::queryCmdSend(qint8 cardNum)
{
	...
	if (uSocket->waitForReadyRead(UDPREADWAIT_TIMEOUT))
	{
		...
		if (datalen >= 93)
		{
			...
			if (datagram.data()[0] == 0xbe && datagram.data()[1] == 0xbe && datagram.data()[7] == 0xbe)
			{
				analysisGainString(datagram);
				g_disText << weChinese2LocalCode("查询命令，数据到达 ！");
			}
			else {
				//g_disText << weChinese2LocalCode("查询命令，未知错误 ！");				
			}
		}
	}
	else 
	{
		g_disText << weChinese2LocalCode("查询命令,等待数据超时 ！");
	}
...
{
	if (m_pTypeLabel[cardNum]->text() == "NULL")
	{
	...
	if (m_pTypeLabel[cardNum]->text() == "AI")
	{
		typeVal = 1;
	}
	else if (m_pTypeLabel[cardNum]->text() == "AO")
	{
		typeVal = 2;
	}
	else if (m_pTypeLabel[cardNum]->text() == "DI")
	{
		typeVal = 3;
	}
	else if (m_pTypeLabel[cardNum]->text() == "DO")
	{
		typeVal = 4;
	}
	else
	...
	{
		int t = m_pGainComBox[(cardNum-1)*MAXSLAVECHANNELNUM + i]->currentIndex();
	...
	//be be be be be be be be 08 44  (板卡号1~16)  01（卡类型） （64个字节的增益信息） crc ff ff ff ff ff ff ff ff ed ed ed ed ed ed ed ed
	relay_data[10] = card[0];
	relay_data[11] = card[1];
	relay_data[12] = type[0];
	relay_data[13] = type[1];

	relay_data[78] = relay_data[76] + card[0]; //crc 
	relay_data[78] = relay_data[76] + card[1];
	relay_data[78] = relay_data[76] + type[0];
	relay_data[78] = relay_data[76] + type[1];
	for (int i = 0; i < 64; i++)
	{
		relay_data[14 + i] = gain[i];
		relay_data[78] = relay_data[78] + relay_data[14 + i];
	}
...
void macSP::analysisGainString(QByteArray data)
{
	...
	if (offline)
	{
		m_pTypeLabel[cardVal]->setText(weChinese2LocalCode("离线"));
		return;
	}

	//data : be be be be be be be be 08 43  01 (板卡号1~16)  （64个字节的状态信息） crc ff ff ff ff ff ff ff ff ed ed ed ed ed ed ed ed
	card[0] = data.data()[10];
	card[1] = data.data()[11];
	cardVal = atoi(card) - 1;
	Type[0] = data.data()[12];
	...
	{
	case 1:
		m_pTypeLabel[cardVal]->setText("AI");
		break;
	case 2:
		m_pTypeLabel[cardVal]->setText("AO");
		break;
	case 3:
		m_pTypeLabel[cardVal]->setText("DI");
		break;
	case 4:
		m_pTypeLabel[cardVal]->setText("DO");
		break;
	default:
		break;
	}
	...
	for (int i = 0; i < MAXSLAVECHANNELNUM;i++)
	{
		for (int j = 0; j < 4;j++)
		{
			gain[j] = data.data()[i*4+j];
		}		 
		gainVal = atoi(gain);
		sprintf(tmp, "增益 G%d", gainVal+1);
		m_pStatusLabel[cardVal*MAXSLAVECHANNELNUM + i]->setText(weChinese2LocalCode(tmp));
	}
}
